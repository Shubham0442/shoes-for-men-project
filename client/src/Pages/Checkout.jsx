...
const Checkout = () => {
  ...
  const handleOrderPlace = () => {
    dispatch(addOrderDetails(addressDone, token)).then((res) => {
    ...
    setTimeout(() => {
      showSetLoad(false);
      dispatch(emptyCart(token)).then((res) => {
        if (res.type === "SET_EMPTY_CART") dispatch(getCart(token));
      });
      orderPlacedToast({
  ...
  useEffect(() => {
    if (deliveryAddress.length === 0) dispatch(getDeliveryAddress(token));
  ...
  return (
    <Box w={"100%"} bg={"#f1f3f6 "} m={"auto"}>
      ...
      >
        <Box w={{ base: "100%", sm: "100%", md: "65%", lg: "65%" }}>
          ...
          {c > 0 ? (
            <Box bg={"white"} pb={"10px"}>
              <Text fontSize={"18px"} pt={"20px"} fontWeight={"550"}>
        ...
        <Box
          w={{ base: "100%", sm: "100%", md: "35%", lg: "35%" }}
          bg={"white"}
        >
          <Text fontSize={"18px"} fontWeight={"550"} mt={"15px"} mb={"20px"}>
            Your Purchase
          </Text>
